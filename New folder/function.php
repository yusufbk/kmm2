...
function getinfopoli($tanggal){
	...
	if($ada > 0){
		...
		$reply = "Jam praktek pada tanggal $tgl_ubah belum tersedia, silahkan hubungi pihak rumah sakit";
...
function update_totalpasien($id){
	$batas2 = mysql_query("select count(a.jadwal_praktek_fk) b from m_jadwal_praktek_antrian as a join m_jadwal_praktek as b on a.jadwal_praktek_fk = b.pk 
							where b.id_session_jadwal = '$id'");
...
function balasan($noreg, $id, $poli){
	$pasien = mysql_query("select * from m_kontak as a join m_jadwal_praktek_antrian as b on a.pk = b.pasien_fk join m_jadwal_praktek as c on 
					b.jadwal_praktek_fk	= c.pk where a.status_login = 'pasien' and a.nomor_registrasi = '$noreg' and c.id_session_jadwal = '$id'");			
